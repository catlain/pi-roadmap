...
const TMP_DIR = path.join(os.tmpdir(), "roadmap-store-supplement-" + process.pid);
...
import { ARCHIVE_DIR, FILE_SUFFIX, GLOBAL_ROADMAP_DIR } from "../lib/types";

describe("store listRoadmapFiles", () => {
	const origDir = GLOBAL_ROADMAP_DIR;
	...
	it("目录不存在时返回空数组", () => {
		...
		const result2 = (() => {
			try {
				...
				return fs.readdirSync(nonExistentDir)
...
describe("store listArchivedFiles", () => {
	...
	it("返回归档文件路径", () => {
		...
		const files = fs.readdirSync(TMP_ARCHIVE_DIR)
			.filter((f) => f.endsWith(FILE_SUFFIX))
...
describe("store readRoadmap", () => {
	...
	it("补全缺失的 meta.tags 字段", () => {
		const data = { meta: { id: "test", title: "测试", status: "active", created: "2026-01-01", updated: "2026-01-01" }, epics: [] };
	...
	it("修复后写回文件", () => {
		const data = { meta: { id: "test", title: "测试" }, epics: [{ id: "E1", status: "badstatus", priority: "urgent" as any, stories: [{ id: "E1.S1", status: "bad" as any, tasks: [{ id: "E1.S1.T1", status: "bad" as any }] }] }] };
		fs.writeFileSync(tmpFile, JSON.stringify(data), "utf-8");
		const loaded = readRoadmap(tmpFile);
		expect(loaded).not.toBeNull();
...
describe("store archiveRoadmap / unarchiveRoadmap / readRoadmapById", () => {
	...
	it("unarchiveRoadmap 恢复文件并标记为 active", () => {
		...
		const archivedRm = { ...SAMPLE_RM, meta: { ...SAMPLE_RM.meta, status: "archived" } };
		fs.writeFileSync(dstPath, JSON.stringify(archivedRm, null, 2) + "\n", "utf-8");
		...
		expect(fs.existsSync(path.join(TMP_DIR, "archive", "test" + FILE_SUFFIX))).toBe(false);
