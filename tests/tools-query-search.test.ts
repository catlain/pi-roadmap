...
describe("searchRoadmapData 基本匹配", () => {
	...
	it("按 title 匹配 Story", () => {
		...
		const storyMatch = results.find(
			(r) => r.matchedType === "story" && r.matchedId === "E1.S1",
		);
	...
	it("按 title 匹配 Task", () => {
		...
		const taskMatch = results.find(
			(r) => r.matchedType === "task" && r.matchedId === "E1.S2.T1",
		);
...
describe("searchRoadmapData 归档项", () => {
	it("默认不搜已归档项", () => {
		expect(
			searchRoadmapData([MOCK_RM], "归档").find((r) => r.matchedId === "E3"),
		).toBeUndefined();
	});

	it("includeArchived=true 包含归档项", () => {
		expect(
			searchRoadmapData([MOCK_RM], "归档", { includeArchived: true }).find(
				(r) => r.matchedId === "E3",
			),
		).toBeDefined();
	});
...
describe("searchRoadmapData ID 匹配", () => {
	it("按 Epic ID 匹配（如 E1）", () => {
		...
		expect(
			results.some((r) => r.matchedId === "E1" && r.matchedType === "epic"),
		).toBe(true);
	});
	...
	it("按 Story ID 匹配（如 E1.S2）", () => {
		...
		expect(
			results.some((r) => r.matchedId === "E1.S2" && r.matchedType === "story"),
		).toBe(true);
	});
	...
	it("按 Task ID 匹配（如 E1.S1.T1）", () => {
		...
		expect(
			results.some(
				(r) => r.matchedId === "E1.S1.T1" && r.matchedType === "task",
			),
		).toBe(true);
	});
	...
	it("部分 ID 也能匹配（如 E1.S）", () => {
		...
		// 应该匹配 E1.S1 和 E1.S2
		expect(
			results.filter((r) => r.matchedType === "story").length,
