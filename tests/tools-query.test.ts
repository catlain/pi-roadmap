...
import { ARCHIVE_DIR, GLOBAL_ROADMAP_DIR } from "../lib/types";
...
const MOCK_RM: RoadmapFile = {
	meta: { id: "test", title: "测试", status: "active", created: "2026-01-01", updated: "2026-01-01", tags: ["test"] },
	epics: [
		{
			id: "E1", title: "Epic 1", description: "", status: "doing", priority: "high",
			project: "/test", stories: [
				{ id: "E1.S1", title: "Story 1", description: "", status: "doing", tasks: [{ id: "E1.S1.T1", title: "T1", status: "todo" }] },
			],
		},
		{
			id: "E2", title: "Epic 2", description: "", status: "done", priority: "medium",
			project: "/test", stories: [
				{ id: "E2.S1", title: "Story done", description: "", status: "done", tasks: [{ id: "E2.S1.T1", title: "T1", status: "done" }] },
			],
		},
		{
			id: "E3", title: "Archived Epic", description: "", status: "done", priority: "low",
			project: "/test", archived: true,
			stories: [{ id: "E3.S1", title: "Archived", description: "", status: "done", archived: true, tasks: [{ id: "E3.S1.T1", title: "T1", status: "done", archived: true }] }],
		},
	],
};
...
describe("registerListTool", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(getOverview).mockReturnValue({
			id: "test", title: "测试", status: "active", tags: ["test"],
			totalTasks: 2, doneTasks: 1, percent: 50, epics: [],
		});
	...
	it("status=archived 时包含归档目录", async () => {
		...
		vi.mocked(getOverview).mockReturnValueOnce({
			id: "archived", title: "测试", status: "archived", tags: ["test"],
			totalTasks: 2, doneTasks: 1, percent: 50, epics: [],
		});

		const result = await execute("call-1", { status: "archived" }, undefined, undefined, undefined);
	...
	it("按 status 过滤", async () => {
		const result = await execute("call-1", { status: "active" }, undefined, undefined, undefined);
		expect(listRoadmapFiles).toHaveBeenCalled();
	});

	it("按 tag 过滤", async () => {
		const result = await execute("call-1", { tag: "test" }, undefined, undefined, undefined);
		expect(result.content[0].text).toContain("测试");
	});
	...
	it("按 tag 过滤：无匹配", async () => {
		...
		const result = await execute("call-1", { tag: "nonexistent" }, undefined, undefined, undefined);
		expect(result.content[0].text).toContain("没有找到路线图");
	});

	it("show_completed=true 时展开已完成 Epic", async () => {
		const result = await execute("call-1", { show_completed: true }, undefined, undefined, undefined);
		expect(result.content[0].text).toContain("Epic 1");
	});

	it("show_archived=true 时显示归档 Epic", async () => {
		const result = await execute("call-1", { show_archived: true }, undefined, undefined, undefined);
		expect(result.content[0].text).toContain("Epic 1");
	});

	it("读取失败时跳过", async () => {
		vi.mocked(listRoadmapFiles).mockReturnValue(["/fake/a.roadmap.json", "/fake/b.roadmap.json"]);
		vi.mocked(readRoadmap).mockReturnValueOnce(MOCK_RM).mockReturnValueOnce(null);
...
describe("registerShowTool", () => {
	...
	it("show_completed 和 show_archived 传参正确", async () => {
		const result = await execute("call-1", { roadmapId: "test", show_completed: false, show_archived: true }, undefined, undefined, undefined);
		expect(formatRoadmapDetail).toHaveBeenCalledWith(
			expect.anything(),
