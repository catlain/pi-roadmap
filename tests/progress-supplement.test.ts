...
function makeStory(overrides: any = {}, tasks: any[] = []): any {
	return { id: "E1.S1", title: "Story", description: "", status: "todo", tasks, ...overrides };
}
function makeEpic(overrides: any = {}, stories: any[] = []): any {
	return { id: "E1", title: "Epic", description: "", status: "todo", priority: "medium", project: "/test", stories, ...overrides };
}

function makeRoadmap(epics: any[] = []): RoadmapFile {
	return {
		meta: { id: "test", title: "Test", status: "active", created: "2026-01-01", updated: "2026-01-01", tags: [] },
		epics,
	} as RoadmapFile;
}
...
describe("calcEpicProgress", () => {
	it("仅有 done 的 epic", () => {
		...
		expect(calcEpicProgress(epic as any)).toEqual({ total: 1, done: 1, percent: 100 });
	});
});
...
describe("calcStoryProgress", () => {
	it("空 story 返回 0%", () => {
		...
		expect(calcStoryProgress(story as any)).toEqual({ total: 0, done: 0, percent: 0 });
	});
	...
	it("部分完成", () => {
		...
		expect(calcStoryProgress(story as any)).toEqual({ total: 2, done: 1, percent: 50 });
	});
});
...
describe("getAllNextTasks", () => {
	...
	it("从多个 active roadmap 合并", () => {
		const rm1 = makeRoadmap([makeEpic({ id: "E1" }, [makeStory({}, [makeTask({ id: "T1", status: "doing" })])])]);
		const rm2 = makeRoadmap([makeEpic({ id: "E2" }, [makeStory({}, [makeTask({ id: "T2", status: "todo" })])])]);
		rm2.meta.id = "rm2";
	...
	it("按 limit 截断", () => {
		const rm = makeRoadmap([makeEpic({}, [makeStory({}, [
...
describe("findTask", () => {
	it("返回找到的 task 及其所在位置", () => {
		const rm = makeRoadmap([makeEpic({}, [makeStory({}, [makeTask({ id: "T1" })])])]);
...
describe("getStoriesForProject", () => {
	it("从多个 epic 中收集 project 匹配的 story", () => {
		const rm = makeRoadmap([
			makeEpic({ project: "/a" }, [makeStory({ id: "E1.S1" }), makeStory({ id: "E1.S2" })]),
