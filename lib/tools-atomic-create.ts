...
export function registerCreateTool(pi: ExtensionAPI) {
	pi.registerTool({
		...
		description: "创建新路线图。只需提供 meta 信息，后续用 roadmap_add_epic 等添加内容。",
		...
		async execute(_tc: string, params: { roadmapId: string; title: string; tags?: string[] }) {
			const newPath = path.join(GLOBAL_ROADMAP_DIR, `${params.roadmapId}${FILE_SUFFIX}`);
